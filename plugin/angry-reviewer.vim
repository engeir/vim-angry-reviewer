...
def numbers_next_to_units(line, index):
    ...
    for number in range(9):
        ...
        if (str(number) + " %" in line) or (str(number) + r" \%" in line):
            mistakes.append(
                f'Line {index + 1}. Percent sign "%" should follow numerals without a space, i.e. {number}%'
            )
    return mistakes
...
def elements(text):
    ...
    for element in ELEMENTS:
        occurrence = entire_text.count(" " + element + " ")
        if 0 < occurrence < 5:
...
def abbreviations(text):
    ...
    for unique_abbreviation in set(filtered_abbreviations):
        if (
            (unique_abbreviation not in ELEMENTS)
            and (unique_abbreviation not in EXCEPTIONS)
            and (unique_abbreviation not in UNITS)
        ):
            occurrence = filtered_abbreviations.count(unique_abbreviation)
            if 0 < occurrence < 5:
...
def references(text):
    ...
    for citation in all_citations:
        ...
        for reference in citation_split:
    ...
    if len(years) > 0:
        ...
        older_than_ten = (
            100 * len([age for age in reference_ages if age > 10]) // len(years)
        )
        older_than_five = (
            100 * len([age for age in reference_ages if age > 5]) // len(years)
        )
        ...
        all_authors_lines = re.findall(
            r"\\author[\[\]abcdefg\* ,\d]*{[^}]+}", entire_text
        )
        names = []
        for author_line in all_authors_lines:
            author_line = re.sub(r"\\author[\[\]abcdefg,\d]*{", "", author_line)
            author_line_split = author_line.split(",")
            for each_author in author_line_split:
...
def intro_patterns(text):
    ...
    for word in OVERUSED_INTRO:
        occurrence = entire_text.count(word)
        occurrence_percentage = occurrence / len(entire_text.split(" "))
        if (0.0012 < occurrence_percentage < 0.002) and (occurrence > 1):
            mistakes.append(
                f'Sentences often start with "{word}". Try alternatives like "{OVERUSED_INTRO[word]}".'
            )
        if occurrence_percentage > 0.002 and occurrence > 1:
            mistakes.append(
...
def it_is_latex_text(text):
    ...
    entire_text = unite_valid_lines(text)
    it_is_latex_text = ("\\begin{document}" in entire_text) or (
        "\\documentclass" in entire_text
    )
...
def comparing_absolutes(line, index):
    '''Check if there are comparative absolutes like "nearly infinite"'''
...
def number_of_times(line, index):
    ...
    all_patterns = re.findall(
        r"((millions|hundreds|thousands|dozens) of (times|samples))", line
    )
    for phrase in all_patterns:
        phrase_wrong = str(phrase[0])
        phrase_corrected = (
            phrase_wrong.split(" of ")[0][:-1] + " " + phrase_wrong.split(" of ")[1]
        )
        mistakes.append(
    ...
    for phrase in all_patterns:
        phrase_wrong = str(phrase[0])
        phrase_corrected = (
            phrase_wrong.split(" ")[0][:-1] + " " + phrase_wrong.split(" ")[1]
        )
        mistakes.append(
...
def in_the_name_of_law(line, index):
    ...
    pattern = re.compile(
        "((T|t)he [^ ]*'s (law|distribution|equation|formula|wavelength|rule|limit|theory|radiation))"
    )
    all_matches = pattern.findall(line)
    mistakes = []
    for match in all_matches:
        mistakes.append(
            f'Line {index + 1}. In "{match[0]}" probably no article "the" is needed.'
        )
    return mistakes
...
def extreme_quantities(line, index):
    ...
    pattern = re.compile(
        "(big|large|small) ((conductivity|conductance|resistance|diffusivity)|(thermal|electrical|interface|boundary) (conductivity|conductance|resistance|resistivity|diffusivity)|frequency|value|temperature|concentration|pressure|altitude)"
    )
    all_matches = pattern.findall(line)
    mistakes = []
    for match in all_matches:
        mistakes.append(
            f'Line {index + 1}. Usually "{match[1]}" is high/low rather than "{match[0]}".'
        )

    # Quantities that should be long or short:
    pattern = re.compile(
        "(big|large|small) (wavelength|lifespan|length|period|time frame|time period|distance|path|mean free path|MFP)"
    )
    all_matches = pattern.findall(line)
    for match in all_matches:
        mistakes.append(
            f'Line {index + 1}. Usually "{match[1]}" is long/short rather than "{match[0]}".'
        )
    ...
    for match in all_matches:
        mistakes.append(
            f'Line {index + 1}. Usually "{match[1]}" is wide/narrow rather than "{match[0]}".'
        )
    return mistakes
...
try:
    ...
    print(
        "[WARNING] g:AngryReviewerEnglish must be 'american' or 'british', (using american english)"
    )
    english_opt = "american"
